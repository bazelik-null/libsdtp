...
/// Single SDTP instance. <br>
/// Contains I/O buffers and config. <br>
...
/// Header section of the packet. <br>
/// Contains:
/// 1. Packet ID
...
/// Standard SDTP packet. <br>
/// Contains packet header and body. <br>
...
/// Creates new SDTP instance and returns pointer to it. <br>
/// Config is copied.
sdtp_instance_t* sdtp_instance_create(const sdtp_config_t* config);
/// Frees and closes instance. <br>
/// Caller must set instance pointer to NULL. <br>
/// Config is not freed.
void sdtp_instance_close(sdtp_instance_t* instance);

// PACKET MANIPULATION //

/// Allocates and constructs a new packet. <br>
...
/// Serializes packet to a newly allocated buffer. <br>
/// out_size receives the size in bytes of the returned buffer. <br>
/// Returned data is in host byte order. <br>
/// Caller must free returned pointer.
uint8_t* sdtp_serialize_packet(const sdtp_packet_t* packet, size_t* out_size);
/// Deserializes raw byte data from a buffer to a newly allocated packet. <br>
/// Returned data is in sender host byte order. <br>
/// Caller must free returned pointer.
...
/// Frees buffer and data. <br>
/// Caller must set buffer pointer to NULL. <br>
...
/// Reads a single packet from the input buffer and returns pointer to it. <br>
/// Doesn't advance buffers after reading <br>
/// Caller must free returned pointer.
